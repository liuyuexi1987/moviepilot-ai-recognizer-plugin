...
前提是智能体所在电脑的浏览器里已经登录过夸克/影巢，并且还能读到有效 Cookie。智能体会按 skill/helper 里的规则调用本机 Cookie 导出工具，并写回 MoviePilot。
...
搜索结果里经常会出现 `智能建议`，它不是让智能体自由发挥，而是插件根据画质、集数、字幕、体积、做种、来源和你的偏好做的选择建议。想稳一点就先看建议，再回编号或用 `1详情` 确认。
