...
这份文件用于把 `AgentResourceOfficer` 交给 WorkBuddy、微信侧智能体或其他外部智能体调用。

## 原则

- WorkBuddy 只负责理解、调度和展示。
- 资源搜索、候选选择、解锁、转存、115 登录状态全部交给 `AgentResourceOfficer`。
- 不在提示词里写 Cookie、Token、API Key。
- 同一个用户或群聊固定使用一个 `session`，例如 `workbuddy:${chat_id}`。
