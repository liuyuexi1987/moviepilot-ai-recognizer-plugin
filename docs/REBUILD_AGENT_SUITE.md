...
- `AIRecognizerEnhancer` 已进入 `0.1.3` 阶段，可直接复用 MoviePilot 当前 LLM 配置，在 `NameRecognize` 阶段做本地结构化兜底，并支持把失败样本直接转成建议、写入动作和样本维护工作流

下一步重点：

1. 继续把影巢签到、用户态、配额态能力评估是否并入 `Agent资源官`
2. 继续打磨 `AIRecognizerEnhancer` 的提示词、失败样本沉淀和识别词建议质量
