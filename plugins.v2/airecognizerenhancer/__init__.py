...
class AIRecognizerEnhancer(_PluginBase):
    ...
    def _record_llm_error(self, title: str, path: str, meta_hint: Dict[str, Any], error: Any, provenance: Optional[Dict[str, str]] = None) -> None:
        try:
            ...
            existing.reverse()
            new_identity = self._sample_identity({"title": title, "path": path, "reason": entry["reason"]})
            existing = [row for row in existing if self._sample_identity(row) != new_identity]
...
""",
    ...
    def _recognize(
        ...
        is_title_only = sample_source_kind == "title_only" or (not sample_source_kind and not path)
        try:
