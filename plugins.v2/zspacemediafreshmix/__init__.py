...
class ZspaceMediaFreshMix(_PluginBase):
    # 插件名称
    plugin_name = "极影视刷新（自用魔改）"
    # 插件描述
    plugin_desc = "按 MoviePilot 最近入库记录刷新极影视，兼容电影/电视剧混合分类与新版极空间 Cookie。"
    # 插件图标
    plugin_icon = "zspacemediafreshmix.png"
    # 插件版本
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "zspacemediafreshmix_"
    # 加载顺序
    plugin_order = 15
    # 可使用的用户级别
    auth_level = 1
    ...
    @staticmethod
    def _parse_classify_names(raw_value: Optional[str]) -> List[str]:
        if not raw_value:
            return []
        names = []
    ...
    @staticmethod
    def _parse_cookie_string(raw_cookie: Optional[str]) -> Dict[str, str]:
        if not raw_cookie:
            return {}
        try:
            cookie = SimpleCookie()
        ...
        parsed = {}
        for item in str(raw_cookie).split(";"):
            if "=" not in item:
                continue
    ...
    def refresh(self):
        ...
        if not self._flushall:
            ...
            movie_libs = self._parse_classify_names(self._moivelib)
            tv_libs = self._parse_classify_names(self._tvlib)

            # 极影视里电影和电视剧分类可以混合在一起：
            # 只填一边时，另一边默认复用已填写的分类列表。
            if not movie_libs and tv_libs:
                movie_libs = tv_libs.copy()
            if not tv_libs and movie_libs:
                tv_libs = movie_libs.copy()

            if "电影" in types_list:
                classify_list.extend(movie_libs)
            if "电视剧" in types_list:
                classify_list.extend(tv_libs)

            classify_list = list(dict.fromkeys([item for item in classify_list if item]))
            if not classify_list:
                logger.error("电影分类名/电视剧分类名均未配置，且未开启刷新全部分类，无法确定需刷新的极影视分类")
                return
    ...
    def __refresh_zspmedia(self,classify_list):
        ...
        if not self._zsphost or not self._zspcookie:
            return False
        cookie = self._parse_cookie_string(self._zspcookie)
        token = cookie.get("token") or cookie.get("zenithtoken")
        device_id = cookie.get("device_id")
        if not token or not device_id:
            logger.error("极影视 Cookie 缺少 token/zenithtoken 或 device_id，无法提交刷新请求")
            return False
        msgtext= None
        total_msgtext = ""
        # 获取分类列表
        ...
        try:
            rsp_body=RequestUtils(cookies=cookie).post_res(list_url)
            res = rsp_body.json()
            ...
            if res and res["code"] == "200":
                if res["data"] and isinstance(res["data"], list):
                    ...
                    for classify in classify_list:
                        ...
                        rescanres = RequestUtils(headers={"Content-Type": "application/x-www-form-urlencoded"}, cookies=cookie).post_res(rescan_url,formdata)
                        ...
                        if rescanres_json["code"] =="200" and rescanres_json["data"]["task_id"]:
                            ...
                            while True:
                                ...
                                resultRep = RequestUtils(headers={"Content-Type": "application/x-www-form-urlencoded"},
                                                        cookies=cookie).post_res(result_url, formdata)
                                ...
                                'content': [
                                    {
                                        ...
                                        'props': {
                                            ...
                                            'placeholder': '多个逗号分割',
                                            'rows': 6,
                                            'hint': '包含电影的极影视分类名，多个逗号分割；若电影和电视剧混放在同一分类，只填这一项也可以'
                                        }
                                    }                                  
                                ]
                            },
                                ...
                                'content': [
                                     {
                                        ...
                                        'props': {
                                            ...
                                            'placeholder': '多个逗号分割',
                                            'rows': 6,
                                            'hint': '包含电视剧、综艺、纪录片、动漫的极影视分类名，多个逗号分割；若和电影混放在同一分类，只填这一项也可以'
                                        }
                                    }
                                ]
                            }
                                ...
                                'content': [
                                    {
                                        ...
                                        'props': {
                                            ...
                                            'text': '- 网盘媒体库路径：MP网盘媒体库路径,可在历史记录或媒体库配置处查看 ' + '\n' +
                                                    ...
                                                    '- 这是基于 gxterry 原版插件整理的自用魔改版' + '\n' +
                                                    '- 仓库说明：https://github.com/liuyuexi1987/MoviePilot-Plugins/tree/main/ZspaceMediaFreshMix',
