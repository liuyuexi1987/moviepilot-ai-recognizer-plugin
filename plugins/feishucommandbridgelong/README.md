...
MoviePilot 的飞书长连接桥接插件。当前定位是兼容/备份入口；新用户更推荐直接使用 `Agent云盘资源整合` 内置的飞书入口。

## 这版的定位

- 保留旧飞书桥接的轻量远程操作体验
- 作为迁移期兼容插件继续可用
- 新功能优先进入 `Agent云盘资源整合`，避免飞书入口和资源执行逻辑继续分叉
- 如果只想装一个插件完成云盘资源整合 + 飞书入口，优先安装并开启 `Agent云盘资源整合` 的内置飞书入口
...
- `旧桥接直连`
  适合保持现有飞书操作习惯，速度快。
- `自动优先新主线，失败回落旧桥接`
  优先委托 `Agent云盘资源整合`，失败再退回旧桥接。
- `仅走 Agent云盘资源整合 新主线`
  调试和后续统一主干时更合适。

日常老环境可以继续用 `旧桥接直连`。新环境建议改用 `Agent云盘资源整合` 内置飞书入口；如果暂时仍使用本插件，建议切到 `仅走 Agent云盘资源整合 新主线`，让资源动作统一落到 Agent云盘资源整合。

## 新推荐入口

`Agent云盘资源整合` 已内置可选 `Feishu Channel`，开启后可以直接接收飞书长连接消息，并复用同一套 `assistant/route`、`assistant/pick`、115 扫码和待任务续跑能力。

迁移建议：

1. 在本插件里先关闭 `启用插件`。
2. 到 `Agent云盘资源整合` 中打开 `启用内置飞书入口`。
...
- 当飞书桥接走 `Agent云盘资源整合` 新主线时，`115登录` 会直接拉起扫码登录流程
...
- 发片名：进入影巢或盘搜搜索流程
- 发 115 / 夸克链接：自动识别并转存，其中 115 链接会优先委托 `Agent云盘资源整合`，确保失败后的待任务、扫码续跑和取消任务都在同一条会话链里
- `path=/目录`、`位置=目录` 都支持
